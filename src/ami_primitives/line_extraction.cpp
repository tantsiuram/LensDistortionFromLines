...
// THIS FUNCTION UPDATE THE SUBPIXEL_CONTOURS OBJECT WITH THE INITIAL LENS DISTORTION MODEL
void updateContoursFromLensDistortion(
  subpixel_image_contours &sp_contours_modifed, /** OUTPUT CONTOUR SUBPIXEL INFORMATION INITIALIZE WITH THE FISRT LENS DISTORTION MODEL*/
...
// THIS FUNCTION ESTIMATE THE FIRST LENS DISTORTION COEFFICIENT
double estimateFirstLensDistortionCoef(
        ...
        const int depth_score, /**  */
        const int height_score, /**  */
        const int width_score, /**  */
        const float distortion_parameter_resolution, /** DISTORTION PARAMETER DISCRETIZATION STEP */
        const float initial_distortion_parameter, /** INITIAL VALUE OF NORMALIZED DISTORTION PARAMETER */
  ...
  for(int k=0; k<depth_score; k++)
  {
    ...
    // #endif
		for(int ind=0; ind<(int)index.size(); ind++)
		{
			...
			if(angle == height_score) 
        angle = height_score-1;
    ...
    for(int l=0; l<nlineas_plus; l++)
    {
      ...
      for(int a=line_angle; a<alpha_max; a++)
      {
        ...
        float val_l = suma_l*media_l;
        float val = suma*media;
        float val_r = suma_r*media_r;
    ...
    // WE SELECT THE MAXIMUM DISTORTION LEVEL
...
// THIS FUNCTION COMPUTE THE POINTS OF THE LINE POINTS FOLLOWING THE DISTANCE OF THE POINTS TO THE LINE
void computePointsLinePts(
  image_primitives &image_primitive_corrected, /** IMAGE_PRIMITIVES OBJECTS FOR CORRECTED POINTS */
  ...
  const double dot_product_min)
{
  for(int k=0; k<nlineas_plus; k++)
  {
	...
	{
		int q=index[ind];
		double x2=sp_contours_modifed.get_x()[q];
		double y2=sp_contours_modifed.get_y()[q];
...
void debuggingPrimitivesOrientationPts(
  image_primitives &image_primitive_original,  /** OUTPUT IMAGE_PRIMITIVES OBJECTS FOR ORIGINAL POINTS */
  image_primitives &image_primitive_corrected,  /** OUTPUT IMAGE_PRIMITIVES OBJECTS FOR CORRECTED POINTS */
  image_primitives &image_primitive,  /** IMAGE PRIMITIVES WHERE LINES AND DISTORTION MODEL IS DEFINED */
  const int width, 
  const subpixel_image_contours &subpixel_contours /** CONTOUR SUBPIXEL INFORMATION */)  
{
...
// WE REMOVE LINES WITH A SMALL NUMBER OF POINTS AND WE JOINT LINES WHICH ARE TOO CLOSE
void removeLines(
  image_primitives &image_primitive_corrected, /** OUTPUT IMAGE_PRIMITIVES OBJECTS FOR CORRECTED POINTS */
  image_primitives &image_primitive_original, /** OUTPUT IMAGE_PRIMITIVES OBJECTS FOR ORIGINAL POINTS */
  const double ami_pi, /** THE PI VALUE */
  ...
  double& dot_product_min)
{
  ...
  if(dot_product_min < 0.95) 
    dot_product_min = 0.95;
  dot_product_min = 0.99;

  float min_line_points = 0.05*image_primitive.get_lines()[0].get_points().size();
  if(min_line_points < 20) 
    min_line_points = 20;
  
  float distance_line_line_min = 2.*distance_point_line_max;
  if(distance_line_line_min > 10) 
    distance_line_line_min = 10.;

  for(int k=0; k<(int)image_primitive.get_lines().size(); k++)
	{
    ...
    for(int l=k+1; l<(int)image_primitive.get_lines().size(); l++)
      ...
      if(fabs(paso) > dot_product_min)
        ...
        double a2 = image_primitive_corrected.get_lines()[l].get_a();
        double b1 = image_primitive_corrected.get_lines()[k].get_b();
        double b2 = image_primitive_corrected.get_lines()[l].get_b();
        ...
        if(m == ((int)image_primitive.get_lines()[l].get_points().size()+1)) 
          continue;
  ...
  const lens_distortion_model& ini_ldm /** INITIAL DISTORTION MODEL */)
{
  ...
  if(distance_resolution<=0 || angle_resolution<=0 )
  {
    ...
    cout << "Discretization parameters lower or equal to 0 " << endl;
  ...
  const int width_score = (int) (1.2*(2.*sqrt((double) width*width+height*height) / distance_resolution+2));
  ...
  // ESTIMATION OF THE FIRST LENS DISTORTION COEFFICIENT
  double max_votation_score = estimateFirstLensDistortionCoef(
          best_distortion_parameter, image_primitive, ini_ldm, xc, yc,
          index, sp_contours_modifed, ami_pi, angle_point_orientation_max_difference,
          distance_resolution, nlineas_plus, distance_point_line_max,
          depth_score, height_score, width_score, distortion_parameter_resolution,
          initial_distortion_parameter, max_norm, paso_angle, angle_resolution,
          dmi, coseno, seno);
  ...
  computePointsLinePts(image_primitive_corrected, image_primitive_original, 
                      image_primitive, nlineas_plus, index,
                      sp_contours_modifed, subpixel_contours,
                      ld, distance_point_line_max, 
                      distance_resolution, dot_product_min);
  
  // DEBUGGING OF THE PRIMITIVES THROUGH THE ORIENTATION OF THE POINTS
  debuggingPrimitivesOrientationPts(image_primitive_original, image_primitive_corrected, 
                                    image_primitive, width, subpixel_contours);
  ...
  // WE REMOVE LINES WITH A SMALL NUMBER OF POINTS AND WE JOINT LINES WHICH ARE TOO CLOSE
  removeLines(image_primitive_corrected, image_primitive_original,
              ami_pi, distance_point_line_max,angle_point_orientation_max_difference, 
              image_primitive, width, subpixel_contours, dot_product_min);
